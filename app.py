...
with col2:
    ...
    st.markdown(
        """
The optimizer minimizes **specific energy consumption** while attempting to satisfy:
...
if st.button("Run Optimization", type="primary"):
    ...
    if result is None:
        st.warning("No feasible solution found under the current constraints. Try increasing target P80 or lowering minimum throughput.")
    else:
        ...
        c1.metric(
            "Power (kW)",
            f'{result["power_kw"]:.1f}',
            f'{result["power_kw"] - baseline["power_kw"]:.1f}'
        )
        c2.metric(
            "P80 (µm)",
            f'{result["p80_um"]:.1f}',
            f'{result["p80_um"] - baseline["p80_um"]:.1f}'
        )
        c3.metric(
            "Throughput (t/h)",
            f'{result["throughput_tph"]:.1f}',
            f'{result["throughput_tph"] - baseline["throughput_tph"]:.1f}'
        )
        c4.metric(
            "Energy Savings (%)",
        ...
        st.subheader("Operating Point Comparison")
